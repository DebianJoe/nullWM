...
int main (int argc, char **argv)
{
    ...
    for (;;){
        ...
        case XCB_BUTTON_PRESS:{
            ...
            xcb_configure_window(display, win, XCB_CONFIG_WINDOW_STACK_MODE, values);
            geom = xcb_get_geometry_reply(display, xcb_get_geometry(display, win), NULL);
            if (1 == e->detail) {
                values[2] = 1; 
                xcb_warp_pointer(display, XCB_NONE, win, 0, 0, 0, 0, 1, 1);
            } else {
                values[2] = 3; 
                xcb_warp_pointer(display, XCB_NONE, win, 0, 0, 0, 0, geom->width, geom->height);
            }
            xcb_grab_pointer(display, 0, root, XCB_EVENT_MASK_BUTTON_RELEASE
        ...
        case XCB_MOTION_NOTIFY:{
            ...
            pointer = xcb_query_pointer_reply(display, xcb_query_pointer(display, root), 0);
            if (values[2] == 1) {/* move */
                geom = xcb_get_geometry_reply(display, xcb_get_geometry(display, win), NULL);
                ...
                xcb_configure_window(display, win, XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, values);
                xcb_flush(display);
            } else if (values[2] == 3) { /* resize */
                geom = xcb_get_geometry_reply(display, xcb_get_geometry(display, win), NULL);
                values[0] = pointer->root_x - geom->x;
                values[1] = pointer->root_y - geom->y;
                xcb_configure_window(display, win, XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT, values);
                xcb_flush(display);
            }
        }
        break;
